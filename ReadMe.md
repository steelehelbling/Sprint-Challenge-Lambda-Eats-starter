...
It is used to make one page apps and not update the entire page any time it wants to change the page. click it and it just changes the urls of the page. usedRouteMatch is used to hook to the parent element.The useRouteMatch hook attempts to match the current URL in the same way that a Route would. It’s mostly useful for getting access to the match data without actually rendering a Route which most basic responsibility is to render some UI when its path matches the current URL.

2. How would you explain form validation to someone who has never programmed before?

It stops you from submitting a form because you entered the wrong information it's always below to tell you what you missed and what to fix.

3. In 1-2 sentences, define end to end testing. 

A end to end test means to use cypress or other programs to test run forms and see if they work. In cypress you run each input though a test and if it comes out positive you then submit the from. This makes it so you can try different things fast and learn more about problems.
